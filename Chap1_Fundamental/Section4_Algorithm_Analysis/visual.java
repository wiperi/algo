...
import Chap1_Fundamental.Section4_Algorithm_Analysis.Ex._8_Two_Sum_Linearithmic;
...
public class visual {
    ...
    private static double Yscale = 0.01;
    ...
    public static double timeTrial(int N) { // 为处理N 个随机的六位整数的ThreeSum.count() 计时
        ...
        int cnt = _8_Two_Sum_Linearithmic.twoSum(a);
    ...
    public static void main(String[] args) { // 打印运行时间的表格
        ...
        for (int N = (int) Xscale; true; N += 100000) { // 打印问题规模为N 时程序的用时
    ...
    private static void rescaleIfNecessary(int N, double time) {
        if (N > Xscale) {
            rescale(N * 1.5, Yscale);
        }
        if (time > Yscale) {
            rescale(Xscale, time * 1.5);
        }
        return;
    }
